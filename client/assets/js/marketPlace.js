...
async function sellCat() {
  ...
  try {
    ...
    if (!marketplaceIsOperator) {
      const contractLink = `<a class="alert-link" href="https://kovan.etherscan.io/address/${MARKETPLACE_CONTRACT_ADD}"
      ...
      const res = await instanceCatContract.methods.setApprovalForAll(MARKETPLACE_CONTRACT_ADD, "true").send();
    }

    pendingNotification();
    const res = await instanceMarketplaceContract.methods.setOffer(priceWeiStr, tokenId).send();
  } catch (err) {
    errorNotification(err);
    console.log(err);
  }
...
async function buyCat(catId, priceEther) {
  ...
  try {
    ...
    const res = await instanceMarketplaceContract.methods.buyCat(catId).send({ value: priceWei });
