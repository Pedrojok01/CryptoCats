...
const API_KEY_RINKEBY = process.env.API_KEY_RINKEBY;
const API_KEY_GOERLI = process.env.API_KEY_GOERLI;
const API_KEY_KOVAN = process.env.API_KEY_KOVAN;
const API_KEY_POLYGON = process.env.API_KEY_POLYGON;
const API_KEY_MUMBAI = process.env.API_KEY_MUMBAI;
const API_KEY_BSC = process.env.API_KEY_BSC;
const API_KEY_BSCTESTNET = process.env.API_KEY_BSCTESTNET;
...
module.exports = {
  networks: {
    ...
    rinkeby: {
      provider: () => new HDWalletProvider(mnemonic, `${API_KEY_RINKEBY}`),
      network_id: 4,
    ...
    kovan: {
      provider: () => new HDWalletProvider(mnemonic, `${API_KEY_KOVAN}`),
      network_id: 42,
      confirmations: 2, // # of confs to wait between deployments. (default: 0)
      timeoutBlocks: 50, // # of blocks before a deployment times out  (minimum/default: 50)
      skipDryRun: true, // Skip dry run before migrations? (default: false for public nets )
